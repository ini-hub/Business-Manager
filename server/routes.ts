...
export async function registerRoutes(
  ...
  app.get("/api/profit-loss/summary", requireRole("owner"), async (req, res) => {
    try {
      ...
      const { serviceRevenue, productRevenue, totalRevenue, costOfGoodsSold, grossProfit } =
      ...
      expenseList.forEach(e => {
        if (e.category?.isSystem && e.category?.name === "Payroll") {
          totalPayrollExpenses += e.amount;
        } else {
          totalOperationalExpenses += e.amount;
        }
      });

      const totalExpenses = totalOperationalExpenses + totalPayrollExpenses;
  ...
  app.post("/api/expenses", requireManagerOrOwner, async (req, res) => {
    try {
      ...
      const expense = await storage.createExpense({
        storeId, title, amount: Number(amount), categoryId, date, notes, receiptUrl, isAutoGenerated: false
      });
