...
// Longest dial codes first, so a 4-digit code (e.g. +234) is tried before
// a 1-digit code that would otherwise match as a false-positive prefix.
const dialCodesByLength: string[] = Array.from(new Set(countryCodes.map((c) => c.dialCode))).sort(
...
function stripPunctuation(phoneNumber: string): string {
...
export function validatePhoneNumber(phoneNumber: string, countryCodeOrDialCode: string): { valid: boolean; error?: string } {
  ...
  const cleanedNumber = stripPunctuation(phoneNumber).replace(/^0+/, "");
...
// Canonical storage form: dial code + local number with the national trunk
// prefix ("0") and any punctuation stripped. Used at signup so new accounts
// are never affected by the with/without-leading-zero ambiguity below.
export function normalizePhoneForStorage(phoneNumber: string, countryCodeOrDialCode: string): string {
  let country = getCountryByCode(countryCodeOrDialCode);
  if (!country) {
    country = getCountryByDialCode(countryCodeOrDialCode);
  }
...
// historical rows - which would require guessing each row's dial code
// boundary from a bare string - lookups generate every leading-zero variant
// of the identifier across all known dial codes, so either form matches.
export function buildPhoneLookupCandidates(identifier: string): string[] {
  const candidates = new Set<string>([identifier]);

  for (const dialCode of dialCodesByLength) {
    if (!identifier.startsWith(dialCode)) continue;
    const local = identifier.slice(dialCode.length);
    ...
    candidates.add(`${dialCode}${withoutLeadingZero}`);
    candidates.add(`${dialCode}${withLeadingZero}`);
    break; // longest matching dial code wins, no need to try shorter ones too
  }
