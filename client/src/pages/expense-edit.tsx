...
import { z } from "zod";
import { ChevronLeft, X, Link2, Lock } from "lucide-react";
...
export default function ExpenseEditPage() {
  ...
  const isAutoGenerated = !!(expense as any)?.isAutoGenerated;
            ...
            <form onSubmit={form.handleSubmit((d) => updateMutation.mutate(d))} className="space-y-5">
              {isAutoGenerated && (
                <div className="flex items-start gap-2.5 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-300">
                  ...
                  <div>
                    <p className="font-medium">Auto-generated by Payroll</p>
                    <p className="mt-0.5 text-xs text-amber-700 dark:text-amber-400">
                      Date and category are locked. To adjust payroll amounts, use the Payroll module.
                    </p>
                  </div>
                </div>
              )}
                ...
                <FormField control={form.control} name="date" render={({ field }) => (
                  <FormItem>
                    <FormLabel className="flex items-center gap-1.5">
                      Date {isAutoGenerated && <Lock className="h-3 w-3 text-muted-foreground" />}
                    </FormLabel>
                    <FormControl><Input type="date" {...field} disabled={isAutoGenerated} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
              </div>
                ...
                <FormField control={form.control} name="categoryId" render={({ field }) => (
                  <FormItem>
                    <div className="flex justify-between items-center">
                      <FormLabel className="flex items-center gap-1.5">
                        Category {isAutoGenerated && <Lock className="h-3 w-3 text-muted-foreground" />}
                      </FormLabel>
                      {!isAutoGenerated && (
