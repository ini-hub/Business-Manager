...
export default function ExpensesPage() {
  ...
  const bulkDeleteExpensesMutation = useMutation({
    mutationFn: (ids: string[]) =>
      runBulkFanOut(ids, async (id, batchId) => {
        const res = await apiRequest("DELETE", `/api/expenses/${id}`, undefined, { "X-Batch-Id": batchId });
    ...
    onSuccess: ({ counts }) => {
      ...
      toast(
        failed === 0
          ? { title: `${deleted} expense${deleted !== 1 ? "s" : ""} deleted` }
  ...
  type ExpenseReportRow = { date: string; title: string; category: string; amount: number };
  const pdfRows: ExpenseReportRow[] = [...filteredExpenses]
    .map((e) => ({ date: e.date, title: e.title, category: e.category.name, amount: e.amount }))
  ...
  const buildPdfReport = (rows: ExpenseReportRow[]) => ({
    ...
    columns: [
      ...
      { key: "amount", header: "Amount", align: "right" as const, format: (r: ExpenseReportRow) => formatCurrency(r.amount) },
    ],
    rows,
    amountKey: "amount",
    formatAmount: formatCurrency,
    unitLabel: "expenses",
    groupBy: (r: ExpenseReportRow) => r.category,
  });
  ...
  const [visibleExpenses, setVisibleExpenses] = useState<ExpenseWithCategory[]>([]);

  const handleExpensesReportExport = () =>
    exportReportToPDF({
      ...buildPdfReport(pdfRows),
      filename: `expenses-${new Date().toISOString().slice(0, 10)}`,
      title: "Expenses Report",
    });

  const handleVisibleExpensesReportExport = () =>
    exportReportToPDF({
      ...buildPdfReport(
        [...visibleExpenses]
          .map((e) => ({ date: e.date, title: e.title, category: e.category.name, amount: e.amount }))
      ...
      filename: `expenses-current-view-${new Date().toISOString().slice(0, 10)}`,
      title: "Expenses Report (Current View)",
    });
              ...
              actions={[
                {
                  ...
                  tone: "destructive",
                  pending: bulkDeleteExpensesMutation.isPending,
                  onClick: () => bulkDeleteExpensesMutation.mutate(selectedIds as string[]),
                },
              ]}
            />
          )}
